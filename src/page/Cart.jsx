...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const calculateTotalAmount = () => {
      ...
      const finalTotalAmount = Number(
        (
          totalPrice +
          (restaurantInfo?.feeDetails?.totalFee / 100 || 0) +
          gstAndCharges
        ).toFixed(2)
      );
  ...
  return cartItems.length === 0 ? (
    <div>
      <img
        className="flex justify-center items-center m-auto md:w-1/4 md:h-1/4 my-12"
        src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto/2xempty_cart_yfxml0"
        alt=""
      />
      <h3 className="text-2xl font-bold text-gray-600 font-Mulish mb-1">
        Your cart is empty
      </h3>
      <p className="font-Mulish text-base font-semibold mb-10">
        You can go to home page to view more restaurants
      </p>
      <Link className="btn-style btn-add" to={"/"}>
        <button className="p-2 rounded-lg shadow-xl font-Mulish font-semibold text-white bg-[#F39A25]">
      ...
      <div className="my-5 mx-auto max-w-md rounded-xl border-2 p-4 border-[#F39A25]">
        <div className="flex justify-between items-center border-b-1 border-[#d3d3d3] mb-1 pb-4">
          ...
          <img
            ...
            src={MEDIA_ASSETS + restaurantInfo.cloudinaryImageId}
            alt=""
          />
        </div>
        ...
        <div className="flex justify-between">
          ...
          <Link to={"/restaurants/" + restaurantInfo.id}>
            <button className="focus:outline-none text-white bg-[#F39A25] hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2 me-2 mb-2 dark:focus:ring-yellow-900">
              Add More Items
            </button>
          </Link>
        </div>
